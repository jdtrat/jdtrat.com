...
metathis::include_meta(
  meta() %>%
  meta_general(
    description = "A presentation for STA379 at Wake Forest University on Bayesian Adaptive Design Optimization (ADO). The presentation begins with a brief introduction to neuroscience and behavioral tasks before covering ADO with a focus on material from Ahn, Gu, Shen, and colleagues (2020). https://doi.org/10.1038/s41598-020-68587-x",
    generator = "xaringan and remark.js"
  ) %>% 
  meta_social(
    title = "Adaptive Design Optimization",
    url = "https://jdtrat.com/talks/ado/slides.html",
    image = "https://jdtrat.com/talks/ado/cover.png",
    image_alt = "The title slide for the Adaptive Design Optimization. It's a navy blue background with the title in gold text. Underneath, the author's name, Jonathan Trattner, and the date originally presented, 3-26-2021, is in white text.",
    og_type = "website",
...
.my-header[
.title[
Crash Course in `r emo::ji("brain")` 
]
]
...
`r emo::ji("one")` .b[Axons] send information


`r emo::ji("two")` .b[Dendrites] receive information


`r emo::ji("three")` .b[Cell body]
]
...
.f1[`r emo::ji("eye")` `r emo::ji("mouth")` `r emo::ji("ear")`]
]
...
`r emo::ji("small_blue_diamond")` Symptoms develop when dopamine-producing cells in the substantia nigra die. 


`r emo::ji("small_blue_diamond")` Affected individuals have low levels of dopamine.

`r emo::ji("small_blue_diamond")` Behavioral changes such as bradykinesia and alterations in impulsivity and motivation.
]
...
`r emo::ji("small_orange_diamond")` Parkinsonian patients have a skewed cost-benefit for expending energy when moving, which may explain bradykinesia.
]
...
`r emo::ji("small_orange_diamond")` Lower dopamine levels in cortical areas involved in decision-making are associated with higher levels of impulsivity.
]
.f6.relative.top--1.pt2.left-1[
...
.o-60.extra-large-emoji[`r emo::ji("door")`]
...
.flex-column.w-40.center[
.extra-large-emoji[`r emo::ji("door")`]
...
What do you choose`r emo::ji("question")`
]
...
`r emo::ji("wave")` Bayesian Statistics
]
...
`r emo::ji("one")` In ADO, experimental designs are updated after every .b[stage], where the .b[most informative design] is chosen for the next stage. 
]

--

.mt4.bg-light-green.ph3.pv2.br3.bb.bt.bw1.b--light-blue.shadow-4.mid-gray[
`r emo::ji("two")` Data is collected in the next stage, analyzed, and a new design is determined for subsequent stages.
]
...
How do we implement this `r emo::ji("question")`
]
...
.f4[`r emo::ji("stop_sign")`] This adjusting procedure ends when the choices were made for  &#36;25 and &#36;12.5 increments. .f4[`r emo::ji("stop_sign")`]
]
...
Intelligent data collection speeds up experiments, potential `r emo::ji("money_bag")`savings
]
...
Computationally harder to implement, both for the `r emo::ji("woman_scientist")` and `r emo::ji("computer")`
]
]
